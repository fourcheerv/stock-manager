...
import { Trash2, Plus, Package, Edit2, AlertTriangle, Minus, BarChart3, RefreshCw, FileDown } from 'lucide-react';
...
export default function StockManager() {
  ...
  const handleAddProduct = () => {
    if (productForm.name && productForm.currentStock && productForm.minStock) {
      if (editingId) {
        setProducts(products.map(p => p.id === editingId ? { ...p, name: productForm.name, currentStock: Number(productForm.currentStock), minStock: Number(productForm.minStock) } : p));
        setEditingId(null);
      } else {
        setProducts([...products, { id: Date.now(), name: productForm.name, currentStock: Number(productForm.currentStock), minStock: Number(productForm.minStock) }]);
      }
  ...
  const handleDestock = () => {
    if (selectedProduct && movementForm.quantity && movementForm.destockedBy && movementForm.intendedFor) {
      ...
      setProducts(products.map(p => p.id === selectedProduct.id ? { ...p, currentStock: newStock } : p));
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-4xl font-bold text-gray-800">Gestion de Stock de Bobinos</h1>
          </div>
        ...
        {showStats && movements.length > 0 && (
          <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
            ...
            <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="border border-gray-200 rounded-lg p-4">
                <h3 className="text-lg font-semibold text-gray-800 mb-3">Top Déstockeurs</h3>
                <div className="space-y-2">
                  {getStatsByDestocker(selectedYear).slice(0, 5).map((item, index) => (
                    <div key={index} className="flex justify-between items-center py-2 border-b border-gray-100">
                      <span className="font-medium text-gray-700">{item.name}</span>
                      <span className="bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-medium">{item.value}</span>
                    </div>
                  ))}
                </div>
              </div>
